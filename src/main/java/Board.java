...
public class Board extends JPanel implements Runnable, Commons {
	...
	public void gameInit() {
		...
		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < 6; j++) {
				Alien alien;
				if (counter == 5) {
					alien = factory.createAlien("ARMORED", alienX + 18 * j, alienY + 18 * i);
					counter = 0;
				} else {
					alien = factory.createAlien("NORMAL", alienX + 18 * j, alienY + 18 * i);
					counter++;
				}
				aliens.add(alien);
			}
	...
	public void gameOver() {
		...
		int finalScore = manager.getCurrentScore();
			...
			while (it.hasNext()) {
				...
				if (alien.isVisible() && shot.isVisible()) {
					if (shotX >= (alienX) && shotX <= (alienX + ALIEN_WIDTH)
						...
						System.out.println("Score: " + ScoreManager.getInstance().getCurrentScore());
					}
				}
			}
		...
		public void keyPressed(KeyEvent e) {
			...
			if (ingame) {
				...
				if (key == KeyEvent.VK_SPACE) {
					ShotManager manager = new ShotManager();
					if (!shot.isVisible()) {
						shot = manager.getShot("DEFAULT", x, y);
					}
				}
			}
		}
